...
def solve(matrix):
    ...
    def bsdr(current_x,current_y):
        ...
        f_p = sd_pos[0]
        s_p = sd_pos[1]
        f_d = find_path(f_p[1],f_p[0])
        s_d = find_path(s_p[1],s_p[0])
    
        if same_distances(find_path(f_p[1],f_p[0]))==same_distances(find_path(s_p[1],s_p[0]))=={}:
    ...
    return move()
print(solve(gen_matrix(10,20,2,3,8,12,2)))
